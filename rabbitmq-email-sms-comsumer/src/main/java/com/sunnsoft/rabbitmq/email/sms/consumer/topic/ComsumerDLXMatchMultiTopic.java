...
@Component
@Slf4j
public class ComsumerDLXMatchMultiTopic {
    /**
     * #通配符死信消息队列
     * @param message
     * @param headers
     ...
     */
    @RabbitHandler
    @RabbitListener(queues = TopicDLXRabbitConfig.TOPIC_MATCH_MULTI_DLX_QUEUE_NAME,containerFactory = "rabbitListenerContainerFactory")
    public void processForManualAckForDLX(Message message, @Headers Map<String, Object> headers, Channel channel) throws Exception {
        try {
            ...
            log.info("(死信交换机)#通配符DLX交换机开始进行消费....获取生产者消息：\n messageId:" + messageId + "    投递的消息:" + msg);
            // 手动ack
